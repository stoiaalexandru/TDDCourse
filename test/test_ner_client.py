...
class TestNerClient(unittest.TestCase):
    ...
    def test_get_ents_returns_dictionary_given_nonempty_string_causes_empty_doc_ents(self):
        ...
        model.returns_doc_ents([])
        ner = NamedEntityClient(model)
        ents = ner.get_ents("Madison is a city in Mioveni")
        self.assertIsInstance(ents, dict)
    ...
    def test_get_ents_given_multiple_ents_serializes_all(self):
        model = NerModelTestDouble('eng')
        doc_ents = [{'text': 'Australia', 'label_': 'GPE'}, {'text': 'Laurent Fressinet', 'label_': 'PERSON'}]
        model.returns_doc_ents(doc_ents)
        ner = NamedEntityClient(model)
        result = ner.get_ents('...')
        expected_result = {'ents': [{'ent': 'Australia', 'label': 'Location'},
                                    {'ent': 'Laurent Fressinet', 'label': 'Person'}], 'html': ''}
        self.assertListEqual(result['ents'], expected_result['ents'])
